  ...
  return (
    <div className="h-auto min-h-screen bg-slate-950 p-6 sm:px-12 sm:pb-12 sm:pt-8 md:p-24 md:px-24 md:pb-24 md:pt-10">
      <div className="text-black pb-4">
        ...
        {/* Display search term */}
        <h2 className="text-xl text-orange-600 mt-4">{search} News</h2>
      </div>

      {/* Displaying fetched news data */}
      <div className="h-auto bg-black text-white rounded-2xl shadow-2xl p-4 sm:p-8 md:p-10">
        <Link to="https://news.google.com/topics/CAAqKggKIiRDQkFTRlFvSUwyMHZNRFZxYUdjU0JXVnVMVWRDR2dKSlRpZ0FQAQ?hl=en-IN&gl=IN&ceid=IN%3Aen"  target="_blank" >
        <h1
          className="w-44 text-orange-600 hover:text-orange-700 cursor-pointer font-bold text-2xl px-3 pb-2 flex items-center"
        >
          Top stories
          <span className="mt-2">
            <svg
              ...
              >
                <div className="w-full h-auto flex flex-col md:flex-row items-stretch gap-4">
                  ...
                  <div className="md:w-2/4 w-full">
                    ...
                    />
                    <h2
                      className="text-xl sm:text-2xl font-serif mb-2 hover:underline cursor-pointer"
                      onClick={() => window.open(article.url)}
                    >
                      {article.title}
                    </h2>
